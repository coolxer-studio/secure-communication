...
function createRoundKeys(keyBytes) {
  ...
  for (index = 0; index < 32; index += 1) {
    values[index + 4] =
      values[index] ^
      keyTransform(values[index + 1] ^ values[index + 2] ^ values[index + 3] ^ CK[index]);
...
export function encryptCbc(plainBytes, keyBytes, ivBytes) {
...
export function decryptCbc(cipherBytes, keyBytes, ivBytes) {
