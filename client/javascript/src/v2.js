...
export function normalizeV2Path(value) {
  if (typeof value !== 'string'
      || value.charAt(0) !== '/'
      || value.indexOf('\n') >= 0
      || value.indexOf('\r') >= 0
...
function validateEnvelope(envelope, session, now, allowedClockSkewMs) {
  var expectedKeys = ['ct', 'cty', 'kid', 'm', 'nonce', 'p', 'seq', 'sid',
    'st', 'suite', 'ts', 'v'];
  ...
  if (normalizeMethod(envelope.m) !== envelope.m
      || normalizeV2Path(envelope.p) !== envelope.p
      || normalizeContentType(envelope.cty) !== envelope.cty) {
    ...
    derived = await subtle.deriveBits(
      {
        ...
        info: new Uint8Array(utf8Encode(
          'SC2/session/' + SUITE + '/' + material.sid))
      },
      hkdfKey,
      72 * 8);
  } catch (error) {
...
export function createV2Codec(session, options) {
  options = options || {};
  ...
  async function decodeResponse(encoded, expected) {
    ...
    if ((expected.sequence != null && envelope.seq !== expected.sequence)
        || (expected.method && envelope.m !== normalizeMethod(expected.method))
        || (expected.path && envelope.p !== normalizeV2Path(expected.path))) {
...
export var V2_INTERNATIONAL_SUITE = SUITE;
export var V2_ENVELOPE_MEDIA_TYPE = 'application/sc-envelope+json';
