...
协议必须运行在 TLS 之上。SDK 不自动信任 `X-Forwarded-Proto`；只有宿主受信代理层
可以把连接归一化为安全请求。原明文业务入口也必须通过网关、网络分区或应用规则阻断，
避免绕过 `/sc/v1/message`。
