...
| 客户端密钥提取 | WebCrypto 与 Android API 23+ 使用不可导出密钥；Android API 21–22 用 Keystore RSA 包装软件 P-256 私钥；iOS 默认把原始私钥材料存入 Keychain；Java/Go 文件身份依赖 owner-only 权限。高安全宿主应实现硬件 KeyStore/HSM 身份存储 |
...
- iOS Keychain、Java/Go 文件权限和 Android API 21–22 包装存储不等同于硬件不可导出私钥；
