...
TLS 始终是强制要求。协议 v1 在 TLS 之上增加应用层身份绑定、认证路由元数据、明确
key ID 和重放控制，但不替代 TLS 的服务器证书验证、流量分析防护或浏览器 Origin
安全模型。
...
flowchart LR
    ...
    Transport --> TLS["TLS 1.2+ network boundary"]
    TLS --> ServerAdapter["Servlet / net/http / ASGI adapter"]
...
| 威胁 | 必需控制 |
| --- | --- |
| 被动或主动网络攻击 | TLS 1.2+、正常证书验证、受管服务端信任锚 |
...
| 客户端密钥提取 | Android Keystore、iOS Keychain/Secure Enclave、不可导出 WebCrypto 密钥 |
