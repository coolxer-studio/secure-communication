...
// code clone from https://github.com/paolostivanin/libbaseencode
/*
 * 用例
 * char *input = "hello sc";
 * char *ek = base32_encode(reinterpret_cast<const unsigned char *>(input), strlen(input), &err, 0);
 * char *dk = reinterpret_cast<char *>(base32_decode(ek, strlen(ek), &err));
 */
