...
async def test_asgi_rewrite_and_encrypted_response():
    ...
    async with httpx.AsyncClient(transport=transport,base_url="http://example.test") as client:response=await client.post(MESSAGE_ENDPOINT,content=json.dumps(e.__dict__,separators=(",",":")),headers={"content-type":ENVELOPE_MEDIA_TYPE})
...
async def test_fastapi_handshake_route_is_strict_and_fail_closed():
    ...
    app=FastAPI();install_fastapi_routes(app,Reject(),Config());transport=httpx.ASGITransport(app=app)
...
@pytest.mark.asyncio
async def test_explicit_tls_policy_rejects_http_handshake_and_message():
    class Reject:
        async def start(self,*args):raise AssertionError("TLS must be checked first")
        async def finish(self,*args):raise AssertionError("TLS must be checked first")
    config=Config(enabled=True,require_tls=True);app=FastAPI();install_fastapi_routes(app,Reject(),config)
    app.add_middleware(SecureCommunicationMiddleware,config=config,messages=None,routes=None)
    transport=httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport,base_url="http://example.test") as client:
        handshake=await client.post(HANDSHAKE_ENDPOINT,json={})
        message=await client.post(MESSAGE_ENDPOINT,content=b"{}",headers={"content-type":ENVELOPE_MEDIA_TYPE})
    assert handshake.status_code==400 and handshake.json()["code"]==TLS_REQUIRED.code
    assert message.status_code==400 and message.json()["code"]==TLS_REQUIRED.code
